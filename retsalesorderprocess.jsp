   ...
   try
	{
		...
		String uid = (String)session.getAttribute("uid");
		 ...
		 Statement check = con.createStatement();
			...
			int WPOID=0;
	     ResultSet rs=st.executeQuery("select max(PID) from RET_PURCHASE1");
		 if(rs.next())
			{
			...
			String insqry = "insert into RET_PURCHASE1 values("+WPOID+",'"+a+"','"+c+"','"+d+"','"+uid+"')";
			System.out.println("ins qry:"+insqry);
			int up = stmtt.executeUpdate(insqry);
			System.out.println("after update");
			if(up==1)
		{
			out.println("Purchase order Sent to Sub Location Incharge with Order Id:"+WPOID);
		     response.setHeader("Refresh","2;URL=./ret_purchase.jsp");

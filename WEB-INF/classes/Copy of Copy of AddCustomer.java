...
public class AddCustomer extends HttpServlet
{
  	public void doPost(HttpServletRequest req,HttpServletResponse resp)throws ServletException,IOException
  	{
		...
		try
		{
			...
			int RETAILERID=0;
			ResultSet rs=stmt.executeQuery("select max(CUSTOMERID) from WHCUSTOMER");
			...
			con.setAutoCommit(false);
			 PreparedStatement pstmt=con.prepareStatement("insert into WHCUSTOMER values(?,?,?,?,?,?,?,?,?,?,?)");
			...
			String qryy = "insert into whm_login values('"+RETAILERID+"','"+cpass+"','5')";
			System.out.println("Qryy:"+qryy);
			i+=st.executeUpdate(qryy);
			if(i==2)
			{
				out.println("<font face=verdana color=red size=3>Customer for Retailer Added Successfully</font>");
				resp.setHeader("Refresh","2;URL=./AddCustomer.jsp");
